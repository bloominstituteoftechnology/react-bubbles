...
A token is used for security decions and to store tamper proof info about the system. 

- [X] What steps can you take in your web apps to keep your data secure?

A private route that only allows user(s) with a token to allow access to pages that displays the data. 

- [X] Describe how web servers work.

A web server will take info from the user such as clicks, form input and other user interactions and process that data then returns a response for what the user is requesting. 

- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

PUT (U in CRUD) and DELETE (D in CRUD)
