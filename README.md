...
- [x] Explain what a token is used for.
        ...
        A token is issued from the server in order to authenticate user credentials and permissions. They are issued as strings of encrypted text, which can be stored on the client in either local or session state storage. The server can identify between which clients have been issued tokens and those that have not.

- [x] What steps can you take in your web apps to keep your data secure?

        The first step one can take in securing web app data is to build Protected Routes around pages with sensitive information.Second would be to require the client to be properly authenticated, usually this can be done through distribution of authentication tokens issued from the server. Authentication tokens ensure that only properly authenticated users have access to the sensitive data stored behind the protected routes. Lastly, including authentication tokens in the headers of all network requests from clients ensures that authentication is ongoing with every request sent to the server.

- [x] Describe how web servers work.

        In short, servers are physical computers that store data files for web sites, pages, and applications. The key to remember is that servers are always on a different computer, mostly in different locations than the computer requesting the information, called the client. The client sends a request over the network to the server, requesting whatever data files are housed on the server. After the server authenticates the client, it will send the files to the client, which will load the files on a local browser.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

         U, which stands for Update. When interfacing with APIs or servers one would make a PUT request which would add data to the server/API, thus updating and re-rendering with the new data.

      	        D, which stands for Delete. When interfacing with APIs or servers one would make a DELETE request which would delete the specified data from the server/API, thus updating and re-rendering with the new data minus what was deleted.
