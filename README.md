...
- [X] Explain what a token is used for.
  ...
  `a token is used to identify a user who is logging into a network only when the user enters the correct credentials.`

- [X] What steps can you take in your web apps to keep your data secure?

  `keep your web apps up to date, make sure that everything is running correctly with backend/server, make sure to use private routes, no error messages in your code.`

- [X] Describe how web servers work.

  `A server is a program that uses HTTP, to serve the files that form our web apps to the users, in response to their requests, which are forwarded by their computers HTTP requests.`
