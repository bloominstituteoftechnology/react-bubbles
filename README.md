...
It is a method of authorization given by a website to verify which features/pages a user of a website can access.

- [X] What steps can you take in your web apps to keep your data secure?

Through the implementation of Client Side Authenication into your server side code.

- [X] Describe how web servers work.

1. A user makes a request to receive some source code from the server,
2. The request to the code, which is stored on the server, is then verified based on the requesters authorization level and either approved or denied,
3. Then the server either returns the requested code to the requester or sends an unauthorized message and denies access to the code the requester is attempting to access.

- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

The (C)reated code is (R)ead by the user of the API, who then (U)pdates the code with something new or (D)eletes the code because it is no longer needed.
